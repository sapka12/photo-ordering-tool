...
public class JpaOrderService implements OrderService {
    ...
    private final PhotoTypeCounterSpecificationBuilder photoTypeCounterSpecificationBuilder = new PhotoTypeCounterSpecificationBuilder();

    @Override
    public List<Item> findAllByUser(long userId, boolean active) {
        return itemRepository.findAll(itemSpec.activeByUser(userId, active));
    }

    @Override
    public Order findOneByUser(long userId) {
        Order order = orderRepository.findOne(orderSpec.activeByUser(userId));
        if (order == null) {
    ...
    private boolean isEmpty(Item item) {
        return photoTypeCounterRepository.count(photoTypeCounterSpecificationBuilder.byItem(item.getId())) < 1;
    }
    ...
    public List<PhotoTypeCounter> findAllPhotoTypeCounterByItem(long id) {
        List<PhotoTypeCounter> counters = photoTypeCounterRepository.findAll(photoTypeCounterSpecificationBuilder.byItem(id));
        validate(counters);
        return counters;
    }
    ...
    private PhotoTypeCounter getOrCreateTypeCounter(Item item, PhotoType photoType) {
        PhotoTypeCounter c = photoTypeCounterRepository.findOne(
                photoTypeCounterSpecificationBuilder.byItemAndPhotoType(item.getId(), photoType));
        if (c != null) {
            return c;
        }
    ...
    private void initAndStartFileMoving() {
        movingService.init(null);
        movingService.start();
    }

    private void stopFileMovingAndClearFolder() {
        movingService.stop();
        purgeTempDir();
    }
    ...
    public void closeActualOrders() {
        initAndStartFileMoving();
        downloadPhotosToFolder();
        stopFileMovingAndClearFolder();
        closeOrders();
    }
    ...
    private void closeOrders() {
        ...
        List<Order> activeOrders = orderRepository.findAll(orderSpec.active());
    ...
    private void downloadPhotosToFolder() {
    ...
    public List<Order> findAllClosedOrderByUser(long userId) {
        return orderRepository.findAll(orderSpec.activeByUser(userId, false));
    }

    @Override
    public List<Item> findOneByOrder(long orderId) {
        return itemRepository.findAll(itemSpec.byOrder(orderId));
    }

    @Override
    public List<PhotoTypeCounter> findOneByItem(long itemId) {
        return photoTypeCounterRepository.findAll(photoTypeCounterSpecificationBuilder.byItem(itemId));
    }
}
